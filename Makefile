...
#
#   cd ~/src/atomic_queue && make T=3 distclean && /bin/time make -kRj$(nproc) T=3 CXXFLAGS=-std=c++23 TOOLSET=gcc-11,gcc-12,gcc-13,gcc-14,clang-18,clang-19,clang-20
...
cxxflags.gcc.release := -O3 -mtune=native -fno-{stack-protector,stack-clash-protection,tree-ch,move-loop-invariants} -falign-functions=64 -DNDEBUG ${cxxflags.gcc.asm.${ASM}}
...
run_benchmarks_ci : ${build_dir}/benchmarks_min
	${chrt_fifo} taskset -c 0-3 /bin/time -f '${TIME}' $< 2>&1
