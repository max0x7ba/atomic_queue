...
cxxflags.gcc := -pthread -march=native -std=gnu++17 -W{all,extra,error,no-{maybe-uninitialized,unused-function}} -g -fmessage-length=0 ${cxxflags.gcc.${BUILD}}
...
cxxflags.clang := -stdlib=libc++ -pthread -march=native -std=gnu++17 -W{all,extra,error,no-unused-function} -g -fmessage-length=0 ${cxxflags.clang.${BUILD}}
